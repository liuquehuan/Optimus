...
class TransactionalWorker:
    ...
    def execute_sql_new_order(self, sql, args, explain=False, analyze=False):
        ...
        if not explain:
            ...
            # self.conn.cursor.execute("select count(*) from orders")
            # print(self.conn.cursor.fetchone())
            return None
...
class DBConn:
    def __init__(self, statement_timeout=0, max_retries=3, retry_delay=1):
        self.conn = None
        self.cursor = None
    ...
    def olap_worker(self, sql, param, timeout=0):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 某些情况下 EXPLAIN (FORMAT JSON) 返回的字符串无法被 psycopg2 自动解析为 JSON
                # 这里记录错误并重试；超过重试次数则返回 None，避免直接抛出中断主流程
                print(f"OLAP worker JSON decode error (attempt {attempt + 1}/{max_retries}): {e}")
