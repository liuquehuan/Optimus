...
if __name__ == "__main__":
    ...
    # train_and_save_model(sys.argv[2], X_train, y_train)
    ...
    choose_latency, scan_latency, join_latency = [], [], []
    ...
    for i in range(20):
        for j in range(22):
            ...
            with open(path, "r") as file:
                ...
                explain_time += end - start
                start = time.time()
                join_hint_id = int(join_reg.predict(plan)[0])
                end = time.time()
                inference_time += end - start
                ...
                # try:
                #     if len(param) == 0:
                #         cur.execute(join_hint + sql)
                #     else:
                #         cur.execute(join_hint + sql, param[i * 5])
                #     plan = cur.fetchone()[0][0]
    ...
    # print("sum latency:", np.sum(join_latency))
    # print("median latency:", np.median(join_latency))
    # idx75, idx95 = int(max(0, len(join_latency) * 0.75 - 1)), int(max(0, len(join_latency) * 0.95 - 1))
    # print("75 latency:", join_latency[idx75])
    # print("95 latency:", join_latency[idx95])
