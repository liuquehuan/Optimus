...
def extract_plan(sample, conflict_operators, oid):
    ## start_time是每个样本的开始时间，min_timestamp是所有样本的开始时间的最小值。用来计算overlap。
    ...
    LEAF_TYPES = ["Seq Scan", "Index Scan", "Custom Scan", "Index Only Scan"]
    leaf_matrix = []
    stack = [plan]
    while stack != []:
        parent = stack.pop(0)
        ...
        ## node_feature第一维是oid
        node_feature = [parent["oid"], run_cost,
                        parent["Plan Rows"], LEAF_TYPES.index(parent["Node Type"]) if parent["Node Type"] in LEAF_TYPES else 4]
        ...
        node_merge_feature = [parent["oid"], start_time + parent["Startup Cost"] / 1e6,
                              start_time + parent["Total Cost"] / 1e6, LEAF_TYPES.index(parent["Node Type"]) if parent["Node Type"] in LEAF_TYPES else 4, run_cost,
                              parent["Plan Rows"]]
        ...
        if parent["Node Type"] in LEAF_TYPES:
            leaf_matrix.append(node_feature + [parent["Relation Name"]])

    return start_time, node_matrix, edge_matrix, conflict_operators, node_merge_matrix, oid, leaf_matrix
